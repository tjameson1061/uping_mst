                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "lasbgt") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "lasblt") {
                                $values2[] = $val1['values'];
                            } else if ($condition == "lasbb") {
                                $values3[] = $val1['values'];
                            } else if ($condition == "lasb") {
                                $values4[] = $val1['values'];
//                                dd($values4);
                            } elseif
                            (
        ...
        $post = (new ShouldBeGreaterThan)->applyFilters($post, $values1, $buyerTierID, 'lasbgt');

        $post = (new ShouldBeLessThan)->applyFilters($post, $values2, $buyerTierID, 'lasblt');

        $post = (new ShouldBeBetween)->applyFilters($post, $values3, $buyerTierID, 'lasbb');

        $post = (new ShouldBe)->applyFilters($post, $values4, $buyerTierID, 'lasb');
