...
class MonthlyOtherExpense
{
    ...
    public function applyFilters($lead)
    {
        ...
        */
                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "moesbgt") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "moesblt") {
                                $values2[] = $val1['values'];
                            } else if ($condition == "moesbb") {
                                $values3[] = $val1['values'];
                            } else if ($condition == "moesb") {
                                $values4[] = $val1['values'];
                            } elseif
                            (
                                !$condition ==
        ...
        $post = (new ShouldBeGreaterThan)->applyFilters($post, $values1, $buyerTierID, 'moesbgt');

        $post = (new ShouldBeLessThan)->applyFilters($post, $values2, $buyerTierID, 'moesblt');

        $post = (new ShouldBeBetween)->applyFilters($post, $values3, $buyerTierID, 'moesbb');

        $post = (new ShouldBe)->applyFilters($post, $values4, $buyerTierID, 'moesb');
//        dd($final_post);
