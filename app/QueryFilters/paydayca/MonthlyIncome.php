                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "misbgt") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "misblt") {
                                $values3[] = $val1['values'];
                            } else if ($condition == "misbb") {
                                $values2[] = $val1['values'];
                            } else if ($condition == "misb") {
                                $values4[] = $val1['values'];
//                                dd($values4);
                            } elseif
                            (
        ...
        $post = (new ShouldBeGreaterThan)->applyFilters($post, $values1, $buyerTierID, 'misbgt');

        $post = (new ShouldBeLessThan)->applyFilters($post, $values2, $buyerTierID, 'misblt');

        $post = (new ShouldBeBetween)->applyFilters($post, $values3, $buyerTierID, 'misbb');

        $post = (new ShouldBe)->applyFilters($post, $values4, $buyerTierID, 'misb');
