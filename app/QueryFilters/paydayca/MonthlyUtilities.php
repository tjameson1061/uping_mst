                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "musbgt") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "musblt") {
                                $values2[] = $val1['values'];
                            } else if ($condition == "musbb") {
                                $values3[] = $val1['values'];
                            } else if ($condition == "musb") {
                                $values4[] = $val1['values'];
//                                dd($values4);
                            } elseif
                            (
        ...
        $post = (new ShouldBeGreaterThan)->applyFilters($post, $values1, $buyerTierID, 'musbgt');

        $post = (new ShouldBeLessThan)->applyFilters($post, $values2, $buyerTierID, 'musblt');

        $post = (new ShouldBeBetween)->applyFilters($post, $values3, $buyerTierID, 'musbb');

        $post = (new ShouldBe)->applyFilters($post, $values4, $buyerTierID, 'musb');
