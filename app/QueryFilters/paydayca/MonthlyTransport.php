                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "mtsbgt") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "mtsblt") {
                                $values2[] = $val1['values'];
                            } else if ($condition == "mtsbb") {
                                $values3[] = $val1['values'];
                            } else if ($condition == "mtsb") {
                                $values4[] = $val1['values'];
//                                dd($values4);
                            } elseif
                            (
        ...
        $post = (new ShouldBeGreaterThan)->applyFilters($post, $values1, $buyerTierID, 'mtsbgt');

        $post = (new ShouldBeLessThan)->applyFilters($post, $values2, $buyerTierID, 'mtsblt');

        $post = (new ShouldBeBetween)->applyFilters($post, $values3, $buyerTierID, 'mtsbb');

        $post = (new ShouldBe)->applyFilters($post, $values4, $buyerTierID, 'mtsb');
