                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "mfsbgt") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "mfsblt") {
                                $values2[] = $val1['values'];
                            } else if ($condition == "mfsbb") {
                                $values3[] = $val1['values'];
                            } else if ($condition == "mfsb") {
                                $values4[] = $val1['values'];
//                                dd($values4);
                            } elseif
                            (
        ...
        $post = (new ShouldBeGreaterThan)->applyFilters($post, $values1, $buyerTierID, 'mfsbgt');

        $post = (new ShouldBeLessThan)->applyFilters($post, $values2, $buyerTierID, 'mfsblt');

        $post = (new ShouldBeBetween)->applyFilters($post, $values3, $buyerTierID, 'mfsbb');

        $post = (new ShouldBe)->applyFilters($post, $values4, $buyerTierID, 'mfsb');
