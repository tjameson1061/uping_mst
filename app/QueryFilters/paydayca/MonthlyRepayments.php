                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "mrsbgt") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "mrsblt") {
                                $values2[] = $val1['values'];
                            } else if ($condition == "mrsbb") {
                                $values3[] = $val1['values'];
                            } else if ($condition == "mrsb") {
                                $values4[] = $val1['values'];
//                                dd($values4);
                            } elseif
                            (
        ...
        $post = (new ShouldBeGreaterThan)->applyFilters($post, $values1, $buyerTierID, 'mrsbgt');

        $post = (new ShouldBeLessThan)->applyFilters($post, $values2, $buyerTierID, 'mrsblt');

        $post = (new ShouldBeBetween)->applyFilters($post, $values3, $buyerTierID, 'mrsbb');

        $post = (new ShouldBe)->applyFilters($post, $values4, $buyerTierID, 'mrsb');
