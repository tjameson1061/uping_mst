...
class NextPayDateDay
{
    ...
    public function applyFilters( $lead )
    {
        ...
        */
                ...
                foreach ($filter as $val) {
                    if (is_array($val)) {
                        foreach ($val as $val1) {
                            ...
                            if ($condition == "npdbal") {
                                $values[] = $val1['values'];
                            } else if ($condition == "npdbam") {
                                $values1[] = $val1['values'];
                            } else if ($condition == "npdsbb") {
                                $values2[] = $val1['values'];
                            } elseif
                            (! $condition == 'npdbal' && 'npdbam' && 'npdsbb') {
        ...
        $post = (new DaysMin)->applyFilters($post, $values, $buyerTierID, 'npdbal');

        $post = (new DaysMax)->applyFilters($post, $values1, $buyerTierID, 'npdbam');

        $post = (new DaysBetween)->applyFilters($post, $values2, $buyerTierID, 'npdsbb');
