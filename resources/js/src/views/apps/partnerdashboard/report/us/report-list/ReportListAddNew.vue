      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <v-select
                v-model="reportData.currentPlan"
                :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                :options="planOptions"
                :reduce="val => val.value"
                :clearable="false"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onSubmit = () => {
      store.dispatch('app-user/addUser', reportData.value).then(() => {
        emit('refetch-data')
