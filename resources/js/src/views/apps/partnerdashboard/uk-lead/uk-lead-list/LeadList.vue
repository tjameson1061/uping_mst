      ...
      >
        ...
        <template #cell(info)="data">
          ...
          <b-tooltip :target="`uk-lead-row-${data.item.id}`" placement="top">
            ...
            <p class="mb-0">Timeout: {{data.item.timeout ? data.item.timeout : 'Not available'}}</p>
            <p class="mb-0">
        ...
        <template #cell(isRedirected)="data">
          ...
          <template #cell(vidLeadPrice)="data">
              <b-media vertical-align="center">
                  ...
                  >£ -</small
                  >
                  <small v-else class="align-text-top text-capitalize text-success font bold"
...
export default {
  ...
  setup() {
    const UK_LEADS = "apps-partner-lead-uk";
    ...
    const statusOptions = [
      ...
      store.dispatch('apps-partner-lead-uk/fetchFilterData')
          .then(response => {
