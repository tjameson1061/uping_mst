...
export default {
  ...
  setup() {
    ...
    const ADVERTISER = 'app-advertiser'

    // Register module
    if (!store.hasModule(ADVERTISER)) store.registerModule(ADVERTISER, advertiserStoreModule)

    // UnRegister on leave
    onUnmounted(() => {
      if (store.hasModule(ADVERTISER)) store.unregisterModule(ADVERTISER)
    })
